...
export function QaPanel({ entry, record }) {
  ...
  // Groups of leftover ink dropped for sitting within one cell of a returned
  // box -- a rule, a dot leader, or a box drawn a shade tight. Shown because a
  // page where this is large and `uncovered` is empty is one to spot check
  // rather than trust. Absent on records written before the filter existed.
  const suppressed = cov?.suppressed_regions ?? null;
        ...
        <StatTile
          ...
          sub={
            uncovered.length
              ...
              : suppressed
                ? `none; ${num(suppressed)} beside a returned box`
      ...
      {!cov ? (
        ...
        <>
          ...
          <p className="muted small">
            ...
            Measured over the full page height and the sides' interior, so a
            header or footer counts in both the numerator and the denominator,
            while the binding and the scanner-bed edge stay out of both —{" "}
            {cov.roi &&
          ...
          {suppressed > 0 && (
            <p className="muted small">
              {num(suppressed)} group{suppressed === 1 ? "" : "s"} of leftover
              ink {suppressed === 1 ? "was" : "were"} not reported, for sitting
              within one cell of a box the model did return — a rule, a dot
              leader, or a box drawn a shade tighter than its glyphs. That is
              not dropped content. If this is large and no region is listed
